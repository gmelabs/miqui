...
* By default, this attribute is enabled (1) for all channels.
* If you do not specify this attribute, Splunk attempts to resolve all AD SID and GUID objects.
...
* There can only be one list form of any whitelist.
* In key/regex form, performs a key specific regex for event inclusion.
...
* If no whitelist is present, all events are whitelisted.

blacklist = <list> | key=regex [key=regex]
* Optional. 
* In list form, tells Splunk which event IDs and/or event ID ranges that incoming events must have 
  in order to be indexed.
* In list form, A comma-separated list of event ID and event ID ranges to exclude (example: 4,5,7,100-200).
* There can only be one list form of any blacklist.
* In key/regex form, performs a key specific regex for event exclusion.
* In key/regex formn, the first character of the regex is the delimeter. Valid regexes look like:
  %regex% *regex* "regex" etc. The only restriction is that the delimiter cannot be within the regex itself.
* If no blacklist is present, there is no effect.
...
* There can be up to 10 whitelist/blacklist settings. Multiple regexes on a single entry are logically ANDed.
